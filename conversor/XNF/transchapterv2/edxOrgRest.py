...
def getCourses(_cms_server,_set_cookie):
    ...
    if courses != []:
        ...
        return getCoursesFicus(_cms_server,_set_cookie)

def getCoursesFicus(_cms_server,_set_cookie):
    '''
    connect to studio server and get a list of the courses where the logged user is admin/instructor
    '''
    url = _cms_server +'/home/'
...
def main():
    ...
    serverTarget = (server for server in settings.SERVERS if server["id"] == "upvxtest").next()
    ...
    copyNode(serverSource['url_studio'], cookiesSource, serverTarget['url_studio'], cookiesTarget, u'block-v1:nachopiqueras+metodos1+2016-01+type@course+block@course',chapterSource)
